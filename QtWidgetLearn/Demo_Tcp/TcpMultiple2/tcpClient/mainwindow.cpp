...
void MainWindow::on_pushButtonEnter_clicked()
{
    ...
    if(status == false)
    {
        ...
        tcpsocket->connectToHost(*serverIP, 5050);
...
//        }
        data.a = 10;
        strcpy(data.name,msg.toUtf8().data());

        tcpsocket->write((char*)&data,sizeof(data));
...
//    QByteArray array = tcpsocket->readAll();
    ...
    ui->textEdit->append(QString(data.name));
    ui->textEdit->append(QString::number(data.a));
}
...
void MainWindow::on_pushButtonSend_clicked()
{
   ...
   // tcpsocket->write((char*)msg.toUtf8().data(), msg.length());
    data.a  =  ui->lineEdit->text().toInt();
    if(ui->lineEditSend->text() == "")
    {
        tcpsocket->write((char*)&data.a,sizeof(int));
    }
    else
    {
    strcpy(data.name,msg.toUtf8().data());
