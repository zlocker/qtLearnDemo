...
//    IconHelper::Instance()->SetIcon(ui->pushButton, QChar(0xf00d), 10);
    //关联关闭按钮
...
void frmMessageBox::SetMessage(const QString &msg, int type)
{
    if (type == 0) {
        ...
        ui->labIcoMain->setStyleSheet(QString("color:%1;background:%2;border:none;border-radius:4px;padding:15px;")
                                      .arg("#FEFEFE").arg("#3498db"));
        IconHelper::Instance()->SetIcon(ui->labIcoMain, QChar(0xf0e5), 18);
        ui->btnOk->setVisible(false);
        ui->btnCancel->setVisible(false);
        ui->lab_Title->setText("网络连接信息");
